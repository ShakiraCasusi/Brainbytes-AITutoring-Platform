...
describe('Auth middleware error handling', () => {
  ...
  test('returns 401 when token is expired', async () => {
    // An expired JWT (signed with correct secret but exp in the past)
...
describe('Chat endpoint error handling', () => {
  ...
  test('handles very long messages gracefully', async () => {
    ...
    // Schema no longer enforces maxlength — long messages are accepted.
    // Accept 200/201 (saved), 400 (validation), or 500 (schema error).
