...
describe('Message model', () => {
  ...
  test('saves or rejects message exceeding 1000 characters depending on schema', async () => {
    const message = new Message({
      text: 'a'.repeat(1001),
      sender: 'user',
      sessionId: 'session-001',
    ...
    // Schema may or may not enforce maxlength depending on version
    try {
      const saved = await message.save();
      expect(saved._id).toBeDefined();
      console.log('Schema allows long messages — maxlength not enforced');
    } catch (err) {
      expect(err.name).toBe('ValidationError');
      console.log('Schema rejects long messages — maxlength enforced');
    }
  });
