...
async function runTests() {
  ...
  try {
    ...
    const beforeResponse = await axios.get(`${API_URL}/chat/history/${SESSION_ID}`);
    console.log('History before restart:', beforeResponse.data);
    console.assert(Array.isArray(beforeResponse.data.messages), 'Messages not returned as array');
    console.assert(beforeResponse.data.messages.length >= 1, 'No messages found before restart');
    console.assert(
    ...
    try {
      ...
      console.error('Make sure Docker is running and the container name is correct.');
    ...
    console.log(`Waiting ${RESTART_WAIT_MS / 1000}s then polling for recovery...`);
    await sleep(RESTART_WAIT_MS);
    const recovered = await waitForBackend();
    console.assert(recovered, 'Backend did not reconnect to MongoDB within timeout');
    console.log('✓ Test 3: PASSED\n');

    // Test 4: Data survived the restart
    console.log('Test 4: Previously saved message still exists after restart');
    const afterResponse = await axios.get(`${API_URL}/chat/history/${SESSION_ID}`);
    console.log('History after restart:', afterResponse.data);
    console.assert(Array.isArray(afterResponse.data.messages), 'Messages not returned as array');
    console.assert(
    ...
    console.log(`Message count after restart: ${afterResponse.data.messages.length}`);
