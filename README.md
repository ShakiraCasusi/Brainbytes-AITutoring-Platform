...
2. **`build` (Build Docker Images)**:
   - *Depends on successful `lint` (`needs: lint`)*.
...
4. **`notify` (Pipeline Status Notifications)**:
   - *Runs regardless of overall success or failure (`if: always()`)*.
...
7. In the search box under *Status checks*, search for and select the specific job identifiers from `main.yml`:
...
4. Add the following secrets:
   ...
   - **`DOCKERHUB_USERNAME`** and **`DOCKERHUB_TOKEN`** *(Optional)*: Set these if you wish to push compiled images to a Docker registry in subsequent weeks.
...
- **Troubleshooting**:
  - Open the **Actions** tab, click on the failed workflow run, and click on the failed job (e.g. *Lint Code*) to inspect logs.
