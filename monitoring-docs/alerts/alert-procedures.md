...
### `ServiceDown`
* **Severity:** Critical (SLA: Immediately)
...
### `HighCPUUsage`
* **Severity:** Warning (SLA: 30 minutes)
...
* **Severity:** Warning (>5%) / Critical (>15%) (SLA: 5 minutes)
...
* **Resolution:** If MongoDB is disconnected, restore connection. If unhandled exception, roll back to the last stable git commit.
...
* **Severity:** Warning (>5%) / Critical (>20%) (SLA: 5 minutes)
* **Immediate actions:**
...
* **Severity:** Warning (>3s) / Critical (>8s) (SLA: 5 minutes)
* **Immediate actions:**
  1. Compare database latency vs AI network call latency:
     ...
     ```
* **Resolution:** Cache AI response classifications, or shorten the conversation history limit inside backend queries.
...
* **Severity:** Warning (p90 > 4s) / Critical (p90 > 10s) (SLA: Immediately)
